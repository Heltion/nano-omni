"""Dense NVFP4 block-scaled attention with FP32 online softmax."""
...
class DenseNvfp4Workload(Workload):
...
class DenseNvfp4Arguments(Arguments):
    query: TensorDesc
    key: TensorDesc
    value: TensorDesc
...
def dense_nvfp4_attention(
    ...
    def build():
        @T.macro
        def attend_block(key_block, first, block, lane, inputs, registers):
            # Literal first=True/False specializes this macro at IR construction.
            ...
            for group in T.unroll(tile_kv_tokens // 32):
                for row in T.unroll(2):
                    ...
                    for col in T.unroll(8):
                        ...
                        if first and num_kv_tokens % tile_kv_tokens != 0:
        ...
        def main(
            ...
            with T.Kernel(query_rows // tile_query_tokens, heads, threads=threads) as (
                ...
                # Establish state from the tail before entering the full-tile pipeline.
                attend_block(
                    T.ceildiv(num_kv_tokens, tile_kv_tokens) - 1,
                    True,
                    block,
                    lane,
                    inputs,
                ...
                for iteration in T.serial(T.ceildiv(num_kv_tokens, tile_kv_tokens) - 1):
                    attend_block(
                        T.ceildiv(num_kv_tokens, tile_kv_tokens) - 2 - iteration,
                        False,
                        block,
                        lane,
                        inputs,
                    ...
                    if query_row + row * 8 < num_query_tokens:
                        for column in T.unroll(16):
                            for item in T.vectorized(2):
                                ...
                                ] = result[row, column * 2 + item] / denominator[row]

        return main.with_attr(
            "global_symbol",
            f"dense_nvfp4_{heads}_{total_heads}_{tile_query_tokens}_{tile_kv_tokens}_{threads}",
        )

    return build()


class DenseNvfp4Kernel(
    Kernel[DenseNvfp4Arguments, DenseNvfp4Workload, DenseNvfp4Config]
):
    name = "dense_nvfp4_attention_d128"
    program = dense_nvfp4_attention
    reference_rtol = 0.03
    reference_atol = 0.03

    @classmethod
    def make_arguments(cls, workload: DenseNvfp4Workload) -> DenseNvfp4Arguments:
        query_rows = -(-workload.num_query_tokens // 128) * 128
        key_rows = -(-workload.num_kv_tokens // 128) * 128
        return DenseNvfp4Arguments(
    ...
    @classmethod
    def make_config(cls, workload: DenseNvfp4Workload) -> DenseNvfp4Config:
        del workload
        return DenseNvfp4Config()

    @classmethod
    def make_workload(cls, arguments: DenseNvfp4Arguments) -> DenseNvfp4Workload:
    ...
    @classmethod
    def tops(cls, arguments: DenseNvfp4Arguments) -> Tops:
    ...
    @classmethod
    def ref_program(cls, arguments: DenseNvfp4Arguments) -> None:
        import torch
        ...
        arguments.output.as_torch()[:, start : start + workload.heads].copy_(
